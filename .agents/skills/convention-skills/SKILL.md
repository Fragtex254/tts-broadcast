...
|-------|---------|---------|--------------|------|
| backend-route | 路由端点 | BACKEND「路由/错误处理/参数校验/响应格式」 | 2026-07-19 | ⚠️ 略超体量阈值，后续考虑拆分 |
| backend-service | 服务/外部 API | BACKEND「服务层规范」+ Codex 健壮性2/4 | 2026-07-19 | ✅ |
| backend-database | schema/迁移/DAL | BACKEND「数据库规范」+ Codex 迁移规范 | 2026-07-19 | ✅ |
| backend-testing | 后端测试 | BACKEND「测试规范」+ Codex 健壮性6 | 2026-07-19 | ✅ |
| debug-logging | 前后端调试日志 | Pino debug logging spec | 2026-06-14 | ✅ |
| frontend-component | 组件/页面 | FRONTEND「组件/路由/TS/无障碍/测试/性能」 | 2026-07-21 | ⚠️ 超体量阈值，优先候选拆分（如分出 frontend-testing） |
| frontend-styling | 样式/设计系统 | FRONTEND「设计系统/动效/响应式」 | 2026-07-19 | ⚠️ 略超体量阈值 |
| frontend-state-data | 状态/数据流 | FRONTEND「状态管理/API/防抖/Settings」+ Codex 健壮性1/3 | 2026-07-19 | ✅ |
| add-persisted-field | 跨栈加字段 | Codex「持久化 Checklist」+ 健壮性5 | 2026-06-13 | ✅ |
...
| 索引一致 | `AGENTS.md` 路由表、registry、`.Codex/skills/` 目录三者对得上 |

## 生命周期操作

- **新建**：出现全新任务类型时，建 `.Codex/skills/<name>/`，补 registry 与 `AGENTS.md` 路由表。
