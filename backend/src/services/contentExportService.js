...
function exportProject({ project, platform }) {
  ...
  if (platform === 'xiaohongshu') {
    const reasons = project.claims.length
      ? project.claims.map((item) => `- AI 整理理由（待核对）：${item.claim.reasoning || 'AI 未整理出理由，请根据原文证据核对'}`).join('\n')
      : '（尚无 AI 整理理由，请根据原文证据核对）';
