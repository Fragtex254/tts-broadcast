  ...
  return (
    <section className="rounded-card border border-card-border bg-white/80 p-5 shadow-card">
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <ActionButton variant="text" size="sm" onClick={() => setIsEditing((value) => !value)}>{isEditing ? '取消' : '编辑资料'}</ActionButton>
      </div>
      {isEditing ? (
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <ActionButton variant="confirm" shape="pill" isLoading={isSaving} loadingLabel="保存中…" disabled={!draft.episodeTitle.trim()} onClick={() => void save()} className="sm:col-span-2">保存资料</ActionButton>
        </div>
      ) : (
