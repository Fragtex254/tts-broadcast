  ...
  return (
    <section className="rounded-card border border-card-border bg-white/80 p-5 shadow-card">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <ActionButton variant="primary" shape="pill" size="sm" isLoading={isAnalyzing} loadingLabel={`${progress.message} ${progress.percent}%`} onClick={onAnalyze}>{claims.length ? '重新分析观点' : '自动提取观点'}</ActionButton>
      </div>
      ...
      <div className="grid gap-3 lg:grid-cols-2">{displayed.map((claim) => (
        <CompactClaimCard
          key={claim.id}
          claim={claim}
