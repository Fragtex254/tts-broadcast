    ...
    <ModalShell isOpen={isOpen} title="观点详情" subtitle={subtitle} onClose={onClose} size="lg" accent="lemon" closeOnBackdrop={!isWorking} closeOnEscape={!isWorking}>
      {isLoading && !claim ? <DetailSkeleton /> : error && !claim ? (
        ...
        <div className="space-y-5">
          <header>
            <div className="flex flex-wrap items-center gap-2 font-body text-[11px] text-ink-soft/55">
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">{claim.topic_tags.map((tag) => <span key={tag} className="rounded-full bg-lilac/25 px-2.5 py-1 font-body text-[11px] text-ink-soft">{tag}</span>)}</div>
          </header>

          <section className="rounded-2xl border border-card-border bg-white/60 p-4">
            <p className="font-body text-[11px] uppercase tracking-wider text-ink-soft/50">这条观点回答的问题</p>
            <p className="mt-2 font-display text-[15px] font-medium leading-relaxed text-ink">{claim.question}</p>
            <p className="mt-4 font-body text-[11px] uppercase tracking-wider text-ink-soft/50">论证与理由</p>
          ...
          <section className="rounded-2xl border border-lemon/40 bg-lemon/10 p-4">
            ...
            <button type="button" disabled={isWorking} onClick={() => void run(() => onOpenEvidence(claim))} className="mt-4 inline-flex items-center gap-1 rounded-xl bg-lemon px-4 py-2.5 font-body text-[11px] font-medium text-ink shadow-btn ui-transition hover:brightness-105 disabled:opacity-40">
          ...
          <section>
            ...
            <button type="button" disabled={isWorking || noteDraft === claim.user_note} onClick={() => void run(async () => { await onUpdate(claim.id, { userNote: noteDraft }); })} className="mt-2 rounded-xl bg-sage px-4 py-2 font-body text-[11px] text-ink shadow-btn disabled:opacity-40">保存笔记</button>
          </section>

          {claim.source_url && <a href={claim.source_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 font-body text-[11px] text-ink-soft underline decoration-card-border underline-offset-4 hover:text-ink">查看播客来源 <ArrowSquareOut aria-hidden="true" size={12} /></a>}
          ...
          <footer className="flex flex-wrap items-center gap-2 border-t border-card-border pt-4">
            <button type="button" disabled={isWorking} onClick={() => void run(async () => { await onUpdate(claim.id, { isStarred: !claim.is_starred }); })} className="inline-flex items-center gap-1 rounded-xl bg-lilac px-3.5 py-2 font-body text-[11px] text-ink shadow-btn disabled:opacity-40"><BookmarkSimple aria-hidden="true" size={13} weight={claim.is_starred ? 'fill' : 'regular'} />{claim.is_starred ? '取消收藏' : '收藏观点'}</button>
            {onAddToProject && <button type="button" disabled={isWorking} onClick={() => void run(() => onAddToProject(claim.id))} className="rounded-xl bg-sage px-3.5 py-2 font-body text-[11px] text-ink shadow-btn disabled:opacity-40">加入「{projectTitle}」</button>}
            {!isConfirmingDelete ? <button type="button" disabled={isWorking} onClick={() => setIsConfirmingDelete(true)} className="ml-auto font-body text-[11px] text-pink hover:text-ink disabled:opacity-40">删除观点</button> : <div className="ml-auto flex items-center gap-2" role="group" aria-label="确认删除观点"><span className="font-body text-[11px] text-ink-soft">确定永久删除？</span><button type="button" disabled={isWorking} onClick={() => void run(async () => { await onDelete(claim.id); onClose(); })} className="rounded-full bg-pink px-3 py-1.5 font-body text-[11px] text-ink shadow-btn disabled:opacity-40">确认删除</button><button type="button" disabled={isWorking} onClick={() => setIsConfirmingDelete(false)} className="font-body text-[11px] text-ink-soft hover:text-ink">取消</button></div>}
          </footer>
        </div>
      ) : null}
    </ModalShell>
