...
> 高频开发规则（设计系统/组件/状态管理/API/路由/动效/响应式/TS/测试）已迁移为按需加载的 skill，见下方「开发规则」索引。开发前请按 `CLAUDE.md` 的「任务 → skill 路由表」调用对应 skill。
...
以下高频开发规则已从本文档迁移为按需加载的 skill。开发前请按 `CLAUDE.md` 的「任务 → skill 路由表」调用对应 skill（`Skill` 工具），不要全量读规范文档。
