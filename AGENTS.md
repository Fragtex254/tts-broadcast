...
- 前端顶级导航按用户任务组织为「工作台 / 内容库 / 音色库 / 设置」；`/editor/:broadcastId` 与 `/transcribe` 是从工作台或内容库进入的上下文任务页，不再作为顶级导航。口播编辑器只以 URL 中的正整数 Broadcast ID 为恢复上下文，从后端重读 Broadcast、音色、来源 Revision 与 Segments；所有入口先持久化或选择 Broadcast 再按 ID 导航，裸 `/editor`、非法或不存在的 ID 只显示明确错误和返回入口，不得降级使用 Zustand 旧稿。转录历史与统计统一归入内容库。
...
- 播客结构化转录中，`transcription_segments` 是不可变 ASR 事实；去重、合并和用户校对只发生在派生 `transcription_turns`，不得反写 Segment。Summary 必须引用已验证的 Segment index，时间范围由后端事实派生；逐字稿校对后旧摘要标记 `stale`。当前不持久化上传源音频；只有 `source_url` 可严格解析为 Bilibili BV/av 视频时，逐字稿 Turn 才可用后端事实 `start_seconds` 驱动官方外链播放器的 `t` 参数定位，其他来源不得伪装成可播放或自行推测时间。宽屏全屏阅读的上下文栏只可展示与当前 Turn 事实时间范围相交的 `speaker_viewpoint`，无交集时必须显示空态，不得用“最近观点”伪造关联；播放器固定在其下方，窄屏再降级到底部。
...
- 内容创作以 `content_projects` 为聚合根：来源通过项目关联进入工作区，Artifact 只保存稿件身份与用途，正文每次显式保存都新增不可覆盖的 Revision；当前稿件由最新 Revision 派生。新增或关联来源、创建 Artifact、追加 Revision 必须刷新项目 `updated_at`。`broadcasts` / `segments` 是兼容期的音频 Render 与可恢复编辑草稿，不得重新成为内容正文的唯一真相。进入口播编辑器先创建 `status=draft` 且不触发 TTS；未关联 Revision、无 Segment/音频的草稿可在原 ID 保存，切分后同一 Broadcast 转为 `pending`。项目口播稿只能指向 `audio_script` Revision，创建前必须逐字核对请求正文与 Revision 内容；项目稿修改必须先新增 Revision，再创建新 draft 并替换 URL，不得覆盖旧来源快照。该关联表示“创建时来源”而非持续正文绑定，后续 segment 精修不得清空或伪造来源。整篇 Render 必须先同步创建 pending 记录再等待 TTS，并以 Broadcast ID 写音频；补偿时先用 DAL 原子确认并删除尚未收口的 pending，只有确认删除成功才可 `cleanAudioFile()`，状态不明时宁可暂留文件也不得破坏 generated Render。Segment 批量生成和单段 regenerate 必须以启动时 `id + broadcast_id + text + style_tag + index` 快照加持久化 `generation_token` 做成功/失败 CAS，恢复遗留 `generating` 时写入新 token，旧请求不得 ABA 覆盖；编辑、重排或删除后的旧结果必须丢弃并补偿清理。Segment 音频文件名使用 Segment ID + generation token，禁止按可变 index 覆盖或重命名，内部 token 不进入公共 DTO。项目删除后 Render 保留并把来源关联置空；缓存 FIFO 只淘汰已生成 Render，不得删除 draft/pending 任务。项目、文本来源、Artifact 与 Revision 不受音频缓存 FIFO 清理影响，破坏性迁移或旧字段清理必须先回填核对并获得用户明确批准。
- 已保存历史 Render 继续编辑时必须派生新 Broadcast，不得复用原 ID。已分段 Render 的副本保留 Segment 文字、顺序、标签和倍速，但必须清空音频、错误与生成 token；整稿编辑、添加开场/结尾和初次切分只对无 Segment 的 `draft` 开放。AI 切分调用期间保持 draft 可刷新，最终必须以启动正文快照 CAS 在同一 DAL 事务内写入 Segments 并转 `pending`；并发编辑后旧结果丢弃，禁止留下 `pending + 空 Segments`。
- 自动化不得把“保存 cron 配置”伪装成业务已执行：只有注入真实执行器并成功完成内容生产后才能更新 `last_run_at`。没有执行器时 scheduler 不启动 cron，前端必须明确标记不可用；后续 Automation Run 需要持久化输入、状态、错误和产物。该功能当前从导航隐藏，属于 backlog；保留 `/automation` 直达路由只为兼容旧书签和查看旧配置。
