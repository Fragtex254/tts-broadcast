...
| 转录聚合（原文、Speaker/Segment/Turn、摘要、任务状态） | SQLite `transcription_*` 表族 | 永久，删除聚合根时级联删除；用户上传源音频不持久化 |
