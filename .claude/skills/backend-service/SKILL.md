...
description: 新增或修改后端服务、封装外部 API（MiMo LLM/TTS/ASR、Embedding、播客观点研究、AI HOT）时使用。涵盖服务职责边界、解构参数签名、禁用全局变量传依赖、外部 API 失败隔离（timeout、401/429/超时/网络错误转中文）、LLM/TTS 全局队列限速、观点证据校验、TLS 按实例隔离、SQLite 与音频文件一致性补偿清理。触发场景：加服务、改 services、观点提取、关系分析、Embedding、接外部 API、调 MiMo/aihot/tts/asr、音频写入清理、孤儿文件、队列限速。
---
