...
| `asr.js` | ASR provider 分发服务，按配置调用 MiMo、Qwen/MLX、WSL 或 MOSS 并合并切片文本/usage | mimo, media, mimoApiClient, qwenAsr, wslAsr, mossAsr |
| `asrModels.js` | OpenAI-compatible ASR 模型列表候选 URL 生成、顺序探测、响应解析 | axios |
| `mossAsr.js` | MOSS OpenAI-compatible ASR 转录服务 | axios |
