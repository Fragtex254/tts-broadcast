...
14. SSE 的 `complete/error` 必须由 `sseManager` 做有界、短期 terminal 回放，覆盖断线重连窗口；首连与重连都允许回放。每轮任务使用独立 taskId，Segment 不得复用 Broadcast ID 作为新任务 taskId；生产端发送任何事件前清同 taskId 的旧快照。回放不是业务持久化，Creation/Summary 等长任务仍以各自 lease/store 为事实源。
...
| `sseManager.js` | SSE 连接、全局关闭与有界 terminal 事件回放 | logger |
...
- [ ] **SSE 断线收口**：terminal 事件可短期回放；每轮任务 ID 唯一且生产端显式清同 ID 旧快照
