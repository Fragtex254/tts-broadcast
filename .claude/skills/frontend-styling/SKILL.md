...
description: 套用 Warm Workbench / Soft Editorial 设计系统、调整样式、改配色或动效时使用。涵盖色彩 token（paper/ink/pink/lemon/blush/sage/lilac）与语义色映射、字体与字号、卡片/按钮/输入框/Pill、低频创作里程碑动效、缓动函数、prefers-reduced-motion、响应式断点。触发场景：调样式、改颜色、创作庆祝反馈、烟花、纸屑、改按钮、卡片样式、字体、动画、动效、配色、Tailwind class、响应式布局、好看一点。
---
...
3. 页面标题、区块标题、正文、长文、元信息与控件标签优先使用 `ui-*` 语义排版 class；两套字体默认都走本机 MiSans 中文字体。
4. 根目录 `DESIGN.md` 是完整设计规范；主要卡片、次级内容区、列表行按层级选用，不让所有内容成为同重量白卡。
5. 动效使用 120/180/240ms token；禁止 `transition-all`、高频入场、长列表 stagger 和 hover 浮起；`prefers-reduced-motion` 规则由 `index.css` 统一处理。
6. 低频创作里程碑是受控例外：只在结果卡附近使用 8–12 个非交互粒子或编辑隐喻，不覆盖全页、不阻塞点击；触发、去重、可关闭和 reduced-motion 规则以 `DESIGN.md`「创作里程碑反馈」为准。
...
| 场景 | 优先 class | 规则 |
|------|------------|------|
| 页面标题 | `ui-page-title` | 28–32px、600 weight |
| 区块标题 | `ui-section-title` | 15–16px、允许换行 |
| 工具正文/说明 | `ui-body` | 13px、1.72 行高 |
| 成稿/摘要/逐字稿 | `ui-reading-body` | 15px、1.9 行高、48rem 上限 |
| 时间/数量/来源 | `ui-metadata` | 11px，不承载主要结论 |
| 按钮/筛选/label | `ui-control-label` | 默认 12px，compact 最低 11px |
| 数字索引 | `font-display text-[18px] font-medium` | “01”、“02” |

关键文字一律不小于 11px；10px 只允许非关键装饰编号，禁止新增 9px。不要用全大写和宽字距处理中文长标签。
...
**按钮交互：** 使用 `.ui-pressable` 或等价的精确属性过渡，保留按压反馈；禁止 hover 上浮。
...
```tsx
<span className="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-body font-medium bg-{color}/{opacity} text-ink">
  ✓ 已完成
</span>
```
...
| Class | 效果 | 用途 |
|-------|------|------|
| `animate-fade-in-up` | 从下方淡入上移 | 仅低频、显著状态切换 |
| `animate-fade-in-left` | 从左侧淡入 | 仅低频上下文面板 |
| `animate-fade-in` | 纯淡入 | 编辑模式切换、内容出现 |
| `animate-breathe` | 透明度呼吸 | 状态指示点 |
| `animate-shake` | 水平抖动 | 错误提示 |
| `animate-scale-bounce` | 缩放弹跳 | 保存成功反馈 |
| 局部里程碑粒子 | 纸屑/连线/盖章的单次短动画 | 仅服务端确认的稀有创作里程碑；不得用于普通保存 |
| `animate-waveform-pulse` | 垂直脉动 | 禁止用于持续播放装饰；仅保留兼容旧代码 |
| `animate-pulse` | Tailwind 内置脉冲 | 骨架屏、加载进度条 |

### 入场原则

Header、Sidebar、筛选、普通卡片和长列表不做交错入场。Modal/Popover 使用共享外壳内置动效，业务组件不要写内联 `animation`。

### 缓动函数

- 入场动画：`cubic-bezier(0.22, 1, 0.36, 1)`（ease-out-expo）
- 通用过渡：只声明实际变化属性，使用 120ms 或 180ms token
- 按钮 hover：`hover:brightness-105`（微妙提亮，不换色）

### `prefers-reduced-motion`

`index.css` 已全局处理：在用户开启“减少动态效果”时移除位移、缩放和持续动画，但保留有意义的短颜色/透明度反馈。组件层面无需重复处理。
...
- [ ] 同类文本优先使用 `ui-page-title` / `ui-section-title` / `ui-body` / `ui-reading-body` / `ui-metadata` / `ui-control-label`
- [ ] 关键文字不小于 11px，长文没有 `line-clamp` 或固定高度裁切
- [ ] 无 `transition-all`、hover 浮起或高频入场；动效遵循 `DESIGN.md`
- [ ] 里程碑粒子局部、短促、不可交互；reduced-motion 下退化为静态 sage 完成卡
- [ ] 响应式按 `lg:` 断点处理单栏→双栏
