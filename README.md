...
AI HOT、转录、内容项目和内容库进入口播编辑器前，会先保存一份不触发 TTS 的编辑器草稿。`/editor/:broadcastId` 会按 ID 恢复稿件、音色、分段和来源版本，因此刷新或直接打开链接也不会丢失编辑上下文。

> 自动化尚未接入真实内容生产执行器，当前从导航隐藏并列入 backlog。`/automation` 只为兼容旧书签和查看旧配置而保留，不会把“保存了 cron”显示成“业务已执行”。
