...
开始任何开发前，先按下表调用对应 skill（`Skill` 工具），不要全量读规范文档。技术栈、目录结构、命名、代码风格等背景仍在 `backend/BACKEND_CONVENTIONS.md` 与 `frontend/FRONTEND_CONVENTIONS.md`。
...
> skill 是 Claude Code 专属机制（`.claude/skills/`）。非 Claude agent 无法发现 skill，可直接查阅对应 skill 目录下的 `SKILL.md`，或 `CONVENTIONS.md` 中的索引行。

### 开发中

- **遵循现有模式** — 代码风格、命名、错误处理、测试模式必须与现有代码一致
- **使用 DAL 层** — 后端路由不直接写 SQL，通过 `*Store.js` 操作数据库
- **使用共享工具** — ID 校验用 `validateId()`，文件删除用 `cleanAudioFile()`，不要内联重复逻辑

### 开发后：产生新约定就同步 skill

- 如果开发引入了**新模式 / 新路由族 / 新组件类型 / 新持久化套路**，调用 `convention-skills` 判断更新哪个业务 skill，并同步其 description、Checklist、registry 与本路由表。
- 数据库字段、外部 API 等影响 `CLAUDE.md` 本文档的变更，同步更新本文件对应章节。
- skill / 文档的更新应与代码变更一起提交，不要留到以后。
...
前后端的高频开发规则已迁移为按需加载的 skill，开发前按上方「⚠️ 开发规范强制要求」的路由表调用对应 skill。背景文档（技术栈 / 目录 / 文件职责 / 命名规范 / 代码风格 / 已解决技术债）保留在 `backend/BACKEND_CONVENTIONS.md` 与 `frontend/FRONTEND_CONVENTIONS.md`，并在其「开发规则」章节列出各规则现归属的 skill。
